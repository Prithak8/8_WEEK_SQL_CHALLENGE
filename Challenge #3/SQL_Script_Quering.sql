...
SELECT T.CUSTOMER_COUNT AS CHURNED_CUSTOMERS,
(T.CUSTOMER_COUNT/COUNT(DISTINCT(T.CUSTOMER_ID)) OVER (PARTITION BY (T.CUSTOMER_ID)))*100 AS PERCENTAGE
FROM(
SELECT COUNT(*) AS CUSTOMER_COUNT
FROM SUBSCRIPTIONS AS S
JOIN PLANS AS P
ON S.PLAN_ID=P.PLAN_ID
WHERE P.PLAN_NAME="CHURN") AS T;
